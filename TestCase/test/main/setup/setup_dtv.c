...
HCSHANDLE InitDataManager(void)
{
	...
	CSTTPrint("---CSFSInit = %d\n",nRet);
	nRet = CSFSMkdir(caDir, 0);
	CSTTPrint("---CSFSMkdir = %d\n",nRet);
	nRet = CSFSMount("dm", caDir, "jffs", 0, "-size 1984");
	CSTTPrint("---CSFSMount = %d, caDir=%s, return %d \n",nRet,caDir, nRet);
 ...
 BOOL DoSearch(void)
{
	...
	if(sTunerInfo.m_eType == EM_UDITUNER_TYPE_QAM)
	{
		...
		sDeliver[0].m_CableDeliver.m_dwFrequency = 490000;
		...
		sDeliver[3].m_CableDeliver.m_dwFrequency = 435000; //674000;
...
{
	//int nErrCode = 0;
	//CSSICReqTableInfo_t sReqTable = {0};
	InitSectionSource();
	InitDataManager();
	DoSearch();
