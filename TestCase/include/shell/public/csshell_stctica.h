...
@{
*/
#ifndef CSSHELL_STCTICA_H
#define CSSHELL_STCTICA_H
...
@see CSShellStCTICaSetParam CSShellStCTICaGetDefaultParam
*/
typedef struct
{
    int                   m_nSize;    ///< 本结构体的大小，必须赋值为sizeof(CSShellStCTICaInitParam_S)
	...
	int (*m_fnSaveFlashAsynAfterAdd)(void);///<CS_CA_TXT_Save_Asyn
	int (*m_fnMMIRequestDetailPPVInfo)(DWORD dwPpId, BOOL bType, BYTE ucListPage);///<CS_CA_MMI_RequestDetailPPVInfo
	int (*m_fnMMIRequestPlayLowLevelProg)(DWORD dwPin);///<CS_CA_MMI_RequestPlayLowLevelProg
}CSShellStCTICaInitParam_S;
...
}CSShellStCTICaConfig_S;

/**
@brief 设置算通CA相关操作参数给MMCP
@param[in] pStCa 算通CA信息及相关操作结构体实例
@return 成功返回TRUE,失败返回FALSE
@note 必须在调用CSShellStCTICaSetConfig之前调用该接口。
@code
//绝大多是情况下按此方式调用此接口即可。
CSShellStCTICaSetParam(CSShellStCTICaGetDefaultParam());
@endcode
*/
BOOL CSShellStCTICaSetParam(const CSShellStCTICaInitParam_S* psStCa);
...
@see CSShellStCTICaGetParam
*/
const CSShellStCTICaInitParam_S* CSShellStCTICaGetDefaultParam(void);

/**
@brief 设置算通CA的相关配置信息
@param[in] psConfig 算通CA相关配置信息的结构体实例
@note 该接口必须在CSShellStCTICaSetParam之后调用
@see CSShellStCTICaConfig_S
*/
void CSShellStCTICaSetConfig(CSShellStCTICaConfig_S* psConfig);
