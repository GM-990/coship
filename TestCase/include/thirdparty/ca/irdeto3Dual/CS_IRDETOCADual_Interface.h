...
#define MAX_SC_NUMBER (2) /*最多支持的智能卡数*/
#define SC_NUMBER_LEN (16) /*智能卡号最大长度*/
#define SC_TYPE_LEN (8) /*智能卡类型的最大长度*/
#define SC_VER_LEN (8) /*智能卡版本的最大长度*/
#define SC_NATION_LEN (8) /*智能卡国家码的最大长度*/

/*ALARM消息的来源，ALARM消息的内容以结构体IRDETO_SRVERRINFO_ST地址的形式做为
参数上抛给应用，结构体中的m_bSource有以下几个取值*/
...
#define CA_DEMUX_MAX (2) /* Demux最大数*/
#define CA_SRV_MAX (3)/*多路解扰的可支持的节目最大数*/

#define CA_IPPV_EVENT_MAX (255)/*IPPV 中可以支持的EVENT最大个数*/
...
typedef enum _BAKMSG_TEXT_EN
{
	...
	CA_BAKMSG_TEXT084 = 84 ,            /*Ci-layer type indicator in ECM header is higher than current built up Ci-layer*/                                    
	CA_BAKMSG_TEXT085 = 85 ,            /*Reserved smartcard retun code*/                                    
	CA_BAKMSG_TEXT086 = 86 ,            /*The card is synchronizing the timestamp with the system*/                                    
	...
	CA_BAKMSG_TEXT092 = 92 ,            /*The viewer is not entitled to watch this service because the ECM timestamp expired*/
#if 1                                  
...
/*[Severity][Code]-[Source] <Text Description>
...
typedef enum _MENU_STATUS_EN
{
	...
	CA_MENU_EXTPRODUCT_LIST,//进入Product Status 菜单
	CA_MENU_MAX_DATA
...
{
	CA_STATUS_OTA_OK, /*表示检测到当前有OTA升级码流存在,所带结构体dwParm2 = (DWORD)&T_OtaCfg*/
	CA_STATUS_OTA_STATUS, /*表示检测OTA信息所处的阶段,所带结构体dwParm2 = (DWORD)&IRDETO_OTASTATUS_ST*/
	CA_STATUS_OTA_FAILURE, /*表示检测OTA信息失败,所带结构体dwParm2 = (DWORD)IRDETO_OTAERROR_EN*/
	CA_STATUS_SMC_IN, /* 智能卡插入, 而且该卡为正确的Irdeto卡*/
	CA_STATUS_SMC_OUT, /*智能卡 拔出*/
...
{
	CA_INFO_SOFTCELL_STATUS = 0, /*CA 内核库信息,所带结构体ddwParm2 = (DWORD)&IRDETO_CASTATUS_ST */
	CA_INFO_SC_STATUS,  /*智能卡信息,所带结构体ddwParm2 = (DWORD)&IRDETO_SMARTCARDSTATUS_ST */
	CA_INFO_MONI_EMM_STATUS = 2, /*单个节目信息菜单中的EMM监听信息,所带结构体ddwParm2 = (DWORD)&IRDETO_MONIEMMSTATUS_ST */
	CA_INFO_MONI_DES_STATUS , /*/单个节目信息菜单中的ECM监听信息,所带结构体ddwParm2 = (DWORD)&IRDETO_MONIDESSTATUS_ST */
	CA_INFO_SRC_EMM_STATUS = 4,  /*单个节目信息菜单中的EMM节目信息,所带结构体ddwParm2 = (DWORD)&IRDETO_EMMSRVSTATUS_ST */
	CA_INFO_SRC_ECM_STATUS , /*单个节目信息菜单中的ECM节目信息,所带结构体ddwParm2 = (DWORD)&IRDETO_DESSRVSTATUS_ST */
	CA_INFO_COMPONENTS_STATUS = 6,/* 已不使用*/
	CA_INFO_SC_PINCODECHANGE_STATUS, /*暂不要求,修改PIN码,所带结构体ddwParm2 = (DWORD)&IRDETO_SCPINCODECHANG_ST */
	CA_INFO_SC_PINCODCHECK_STATUS = 8, /*暂不要求,修改PIN码后的状态,所带结构体ddwParm2 = (DWORD)&IRDETO_SCPINCODECHANG_ST */
	CA_INFO_REMOTECHANNEL_STATUS,/*暂不要求,REMOTECHANNEL 功能,所带结构体ddwParm2 = (DWORD)&IRDETO_REMOTECHANNEL_ST */
	CA_INFO_IRD_STATUS = 10, /*暂不使用*/ 
	CA_INFO_LOADER_STATUS, /*Loader 菜单信息,所带结构体ddwParm2 = (DWORD)&IRDETO_LOADERSTATUS_ST */
	CA_INFO_MEMORY_ERROR = 12, /*不用处理,内存错误消息*/
	CA_INFO_SC_NUMSECTOR_STATUS, /*暂不要求,智能卡Sector信息,所带结构体dwParm2 = (DWORD)&IRDETO_SC_NUMBEROFSECTOR_ST*/
	CA_INFO_CAREGION_STATUS = 14,/*暂不要求,智能卡区域码信息,所带结构体dwParm2 = (DWORD)&IRDETO_CAREGION_ST*/
	CA_INFO_SCUSERDATA_STATUS ,  /*暂不要求,智能卡用户信息,所带结构体dwParm2 = (DWORD)&IRDETO_SC_USERDATA_ST*/
	CA_INFO_EMM_USERLOAD_STATUS = 16, /*暂不要求,emm user payload data,所带结构体dwParm2 = (DWORD)&IRDETO_EMM_USERPAYLOAD_ST*/
	CA_INFO_DVBSERVICE_STATUS , /*/CA节目信息汇总菜单,所带结构体dwParm2 = (DWORD)&IRDETO_DVBDESCRABLE_ST*/
	CA_INFO_MAIL = 18, /*邮件和屏显信息,所带结构体dwParm2 = (DWORD)&IRDETO_CAMSG_ST*/
	...
	CA_INFO_MR_CARD_STATUS=22,/*用户进入MR智能卡信息菜单时,需要显示的信息，所带结构体dwParm2 = (DWORD)&MrCardInfo_S*/
	CA_INFO_MR_USERPROFILE_DATA,/*MR设置成功后传递用户的密码等信息，所带结构体dwParm2 = (DWORD)&MrUserProfileData_S*/
	CA_INFO_IPPV_CHECK,/*cxj add IPPV event 是否购买信息，所带结构体为dwParm2 = (DWORD)&IppvEventsCheck_S*/
	CA_INFO_EXT_PRODUCT_LIST,/*Product Status菜单信息，所带结构体*/
	CA_INFO_MAX
}IRDETO_CAINFO_EN;

typedef enum _CACOMMAND_EN
{
	CA_COMMAND_FINGER_PRINT = 0, /*指纹命令,所带结构体dwParm2 = (DWORD)&IRDETO_CACOVERTFINGERPRINT_ST*/
...
/*邮件内容最大长度*/
#define CA_MAX_SIZE_TXT_MAIL       (512*2)
...
/* CA Loader菜单信息，m_stSignDateTime 和m_stDownloadDateTime信息IRDETOCA不作要求显示，APP可自行选择*/
...
/*实现 REMOTECHANNEL 功能，IRDETO暂不作要求，APP可暂不作处理*/
...
/* 智能卡sector分区信息，IRDETO暂不作要求，APP可暂不作处理*/
...
/*智能卡区域码信息，IRDETO暂不作要求，APP可暂不作处理*/
...
/*TMS消息内容最长字节数*/
#define MAX_ATTRIBDATA_LEN (511*2)
...
typedef struct _IppvPreviewInfo
{
	...
	BYTE     m_ucSector;//IPPV Event 所在的Sector分区,0: IPPV节目，1:LPPV节目，需要在界面上标明节目类型
	WORD     m_wEventId;//EventId
	WORD     m_wEventCost;//Event费用
	BYTE     m_ucClockValue;//If bClockValue < bPreviewValue, the event is in PREVIEW，Otherwise the event is in NO-VIEW
	BYTE     m_ucPreviewValue;//节目预览阶段,CW可以正常设置,否则不允许观看
	WORD     m_wServiceCount;//Number of services currently viewing this IPPV event.
}IppvPreviewInfo_S;
...
/*检查某个IPPV Event是否已经被购买*/
typedef struct
{
	BYTE m_ucSector;;//sector号
	WORD m_wEventID;//IPPVEventID
}IPPVEvent_S;

typedef struct
{
	IPPVEvent_S m_sIPPVEvent;
	BOOL m_bSectorValid;
	BOOL m_bEventValid;
	WORD m_wDateCode;
	BOOL m_bReported;
...
typedef struct
{
	DWORD m_dwResourceId;//资源号
	BYTE m_ucStatus;
	BYTE m_ucIPPVEventCount;
	BYTE m_ucGrace;
	IppvEventCheck_S m_sIPPVEvent[CA_IPPV_EVENT_MAX];	
}IppvEventsCheck_S;
...
/*cxj add for product status menu*/
typedef struct
{
	WORD m_wProductID;			
	WORD m_wStartDate; 			
	BYTE  m_ucDuration;				
}CaSectorExtProduct_S;

typedef struct
{
	BYTE m_ucSector;
	BYTE m_ucSectorStatus;
	WORD m_wSectorDateCode;
	BYTE m_ucProductCount;	
	WORD m_wMaxSizeExtendProducts;
	WORD m_wOrgSizeExtendProducts;
	CaSectorExtProduct_S* m_psExtendProducts;
}CaSectorExtSector_S;

typedef struct
{
	WORD m_wResourceId;				/* IN */
	BYTE m_ucSectorCount; 				/* IN */
	WORD m_wMaxSizeSectorProducts;		/* IN */
	WORD m_wOrgSizeSectorProducts;		/* OUT */
	CaSectorExtSector_S* m_psSectorProducts;			/* OUT */
	BOOL m_bExpiredProductsExist;
} CaExtProductList_S;
...
*Parameters 
*	nIndex[in]:邮件存储的下标，从0开始
*	bLockFlag[in]:如果未TRUE,则设置邮件为锁定状态，FALSE,则设置邮件
			     为非锁定状态，默认情况下，邮件都是处于非锁定状态
...
*Parameters 
*	nIndex[in]:邮件存储的下标，从0开始
*	pbLockFlag[out]:TRUE ，则该邮件已锁定，FALSE，则该邮件未锁定
*
*Return Values: 
*	0 : SUCCESS 
*	-1: FAILURE
...
* 获取升级时所需的参数
* 界面（APP）收到CA发送的CA_COMMAND_OTA_DETECT消息或者通过菜单触发OTA升级时，首先调用该函数来获取升级流的频点参数。
*Parameters 
*	无
*
*Return Values: 
...
* 获取升级过程中界面需提示的信息
* 界面（APP）收到CA发送的CA_COMMAND_OTA_NEXT消息时，调用该函数来获取升级流的频点参数
*Parameters 
*	无
*
*Return Values: 
...
//这个先不用调
INT CSCAIPPVEventCheck(DWORD dwResourceId,BYTE bDaysGrace, BYTE bIPPVEventCount, IPPVEvent_S* pIPPVEventToCheck );

/**************************************************************************
*Function Name: CSCAIPPVCallBackStart
* 启动IPPV回传
*
*Parameters 
*	无
...
*		-1: FAILURE
**************************************************************************/
INT CSCAIPPVCallBackStart(void);
...
*  用于多路解扰. 
*
*  注意:收到E16消息时，参数dwParm2即为关闭的音视频类型，可直接通过该参数获取，不需要再调用该接口
*
*Parameters 
*	dwGroupHandle: 当收到E16消息时,参数dwParm1是IRDETO_CA_ERROR_INFO_ST类型的结构体变量地址,应用将该参数强制转换成IRDETO_CA_ERROR_INFO_ST类型后传递其成员m_wServiceHandle到该接口.
*
*Return Values: 
        -1 : FAILURE
*		0 : 音视频全部关闭 
...
/**************************************************************************
*Function Name: CS_CA_SOLParseNitData
...
/**************************************************************************
*Function Name: CSCAGetCurrentUTCTime
*  
*
*Parameters 
*	pucIPAddr[out]: 当前的时间，直接返回TDT表里面的UTC时间即可;
*	ucBuflen[in]:pucIPAddr空间大小
*Return Values: 
*	 成功: SUCCESS
        失败: FAILURE 
*		
**************************************************************************/
INT CSCAGetCurrentUTCTime(BYTE* pucCurTime, BYTE ucBuflen);
#ifdef  __cplusplus
}
#endif
