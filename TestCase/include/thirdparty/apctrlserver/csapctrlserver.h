...
#define CSAPCTRLS_IP_LEN (16)				///<IP地址最大长度,形如"192.168.1.1"
#define CSAPCTRLS_IP_RANGE_LEN (24)			///<Ip地址范围，形如"192.168.1.2~254"
#define CSAPCTRLS_MAC_LEN (6)				///<MAC地址长度，形如{0x00,0x0e,0x09,0x11,0x12,0xe1}
#define CSAPCTRLS_URL_LEN (256)				///<URL长度
 ...
 * */
typedef struct{
	char aucILIp[CSAPCTRLS_IP_LEN]; ///< 源ip地址,空字符串""表示不关心源Ip地址
	unsigned char aucILMac[CSAPCTRLS_MAC_LEN];	///<源mac地址，全为0，表示不关心源mac地址。
	char aucDesinationIp[CSAPCTRLS_IP_LEN];///< 目标ip地址,空支付串""表示不关心目标地址
	CSAPCTRLSProType_E eApplyProtocolType;
	unsigned int nILPortLowerLimit;   ///<源端口上限，当eApplyProtocolType==EM_APCTRLS__PROTOCOL_TYPE_ALL时，端口范围无效
	unsigned int nILPortUpperLimit;   ///<源端口下限，当eApplyProtocolType==EM_APCTRLS__PROTOCOL_TYPE_ALL时，端口范围无效
	unsigned int nDesPortLowerLimit;  ///<目的端口上限，当eApplyProtocolType==EM_APCTRLS__PROTOCOL_TYPE_ALL时，端口范围无效
	unsigned int nDesPortUpperLimit; ///<目的端口下限，当eApplyProtocolType==EM_APCTRLS__PROTOCOL_TYPE_ALL时，端口范围无效
 ...
 * @note:
 * 		Qos规则只有在CSAPCTRLSEnableQos(1)时才会生效
 * */
 typedef struct{
	char aucILIpLowerLimit[CSAPCTRLS_IP_RANGE_LEN]; ///< 本地地址范围，形如"192.168.1.1~254"
	unsigned int nILPortLowerLimit;
	unsigned int nILPortUpperLimit;
	CSAPCTRLSProType_E eApplyProtocolType;
 ...
 * @note
 *     静态路由规则只有在CSAPCTRLSEnableStaticRoute(1)时才会生效
 * */
typedef struct{
	char aucDestinationOGIp[CSAPCTRLS_IP_RANGE_LEN]; ///<若指定目标主机，则字符串格式为"192.168.1.5"；若指定目标网络，则字符串格式为"192.168.1.0/24"
	char aucGatewayIp[CSAPCTRLS_IP_LEN];//网关ip
...
typedef struct{
	...
	CSAPCTRLSRuleType_E eRuleType;///<路由规则类型
	union{
		CSAPCTRLSRuleFilter_S stIpFilter; 		///<基于ip和mac的过滤规则，eRuleType==EM_APCTRLS_RULETYPE_FILTER时有效
		CSAPCTRLSRuleFilterUrl_S stURlFilter;	///<基于url的过滤规则，eRuleType==EM_APCTRLS_RULETYPE_URL_FILTER时有效
		CSAPCTRLSRuleNatIpPort_S stNatIpPort; ///<NAT单端口转换规则，eRuleType==EM_APCTRLS_RULETYPE_NAT_IPPORT时有效
		CSAPCTRLSRuleNatPortRange_S stNatRange;///<NAT端口段转换规则,eRuleType==EM_APCTRLS_RULETYPE_NAT_PORT_RANGE时有效
		CSAPCTRLSRuleNatApp_S stNatApp;		///<特殊应用程序转换规则.eRuleType==EM_APCTRLS_RULETYPE_NAT_APP时有效
		CSAPCTRLSRuleQosSeparately_S stQos;	///<主机独立QOS规则，eRuleType==EM_APCTRLS_RULETYPE_QOS_SEPARATELY时有效
		CSAPCTRLSRuleQosTotal_S stQosTotal;	///<全局QOS规则,eRuleType==EM_APCTRLS_RULETYPE_QOS_TOTAL时有效
		CSAPCTRLSRuleStaticRoute_S stStaticRoute;///<静态路由规则,eRuleType==EM_APCTRLS_RULETYPE_STATIC_ROUTE时有效
	}uRule;
}CSAPCTRLSRule_S;


typedef enum{
	EM_APCTRLS_MODE_SINGL_CPE = 0x1, ///<工作模式：单CPE模式
	EM_APCTRLS_MODE_WIFI_SUBNET = 0x2, ///<工作模式：无线子网模式
	EM_APCTRLS_MODE_WIFI_BRIDGE = 0x3,///<工作模式：无线桥接模式
}CSAPCTRLSMode_E;
...
/**
@brief 获取规则
@param[in] nRuleTypeMask 需获取的规则掩码，由CSAPCTRLSRuleType_E中定义的各个枚举值与运算得到
...
/**
@brief 获取规则信息
@param[in] hRuleHandle 规则句柄，通过CSAPCTRLSGetRules接口获取得到
